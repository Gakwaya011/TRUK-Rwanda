    ...
    <header className="fixed left-0 right-0 mx-auto top-4 z-50 max-w-[1200px] px-4 sm:px-6 lg:px-8 font-sans">
      <div className={`px-6 py-3 flex items-center justify-between rounded-2xl transition-all duration-500 ease-in-out ${navContainerStyle}`}>
        ...
        <nav className="hidden md:flex items-center space-x-8">
          ...
          >
            <Link to="/services" className={`flex items-center gap-1 text-[13px] font-bold uppercase tracking-wide transition cursor-pointer ${textColor}`}>
              Our Services
          ...
          <Link to="/careers" className={`text-[13px] font-bold uppercase tracking-wide transition ${textColor}`}>Careers</Link>
          <Link to="/contact" className="hidden md:block px-5 py-2.5 text-xs font-bold text-black bg-[#FAD201] rounded-full hover:bg-white hover:text-trukGreen transition shadow-lg hover:shadow-xl transform hover:-translate-y-0.5">Contact Us</Link>
        </nav>

        {/* MOBILE TOGGLE */}
        <button onClick={() => setIsOpen(!isOpen)} className={`md:hidden p-2 rounded-lg transition ${isHome && !scrolled ? "text-white hover:bg-white/10" : "text-gray-800 bg-gray-100"}`}>
          ...
          >
            <div className="px-6 py-6 space-y-4">
              ...
              <div className="pl-4 border-l-2 border-gray-100 space-y-3">
                ...
                {serviceLinks.map((subItem) => (
                   <Link key={subItem.name} to={subItem.path} className="block text-sm font-medium text-gray-600 hover:text-trukGreen" onClick={() => setIsOpen(false)}>
                     {subItem.name}
                   </Link>
                ))}
              </div>
              ...
              <Link to="/contact" className="block w-full py-3 text-center text-sm font-bold text-black bg-[#FAD201] rounded-lg shadow-md mt-4 hover:bg-gray-100" onClick={() => setIsOpen(false)}>Contact Us</Link>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
